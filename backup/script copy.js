...
widget.addEventListener("click", () => {
  ...
  if (isOn) {
    ...
    // Call Blynk API to turn on the light
    fetch('https://sgp1.blynk.cloud/external/api/update?token=S0VtFsLDY9YFPVu-2StyyliF1vOgeRFQ&v1=1')
      .then(response => {
        if (!response.ok) {
    ...
    // Call Blynk API to turn off the light
    fetch('https://sgp1.blynk.cloud/external/api/update?token=S0VtFsLDY9YFPVu-2StyyliF1vOgeRFQ&v1=0')
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
